...
- `TASK_NAME`: short identifier for hook-based audit logging (e.g. `brainstorming-skill-md`). If missing, derive from SOURCE_PATH: `{parent_dir_basename}-{file_basename}` lowercased, non-alphanumeric replaced with `-`.

If you cannot determine `SOURCE_PATH`, stop and report the gap. Do not invent paths.

**Hook audit registration (do this as your very first Bash call, before any Read/Write):**

```bash
echo "TASK:{TASK_NAME}"
```

Replace `{TASK_NAME}` with the actual value. This one-liner registers a `session_id → TASK_NAME` mapping with the PostToolUse hook so the hook's per-session counter can be retrieved by task name at the end.
...
You have a HARD budget of `TOOL_BUDGET` tool calls (every Read / Write / Bash / Edit counts, including the hook-registration and budget-check Bash calls). **Do NOT count tool calls in your head — LLM self-counting drifts and is unreliable.**

Instead, use this **budget-check command** at critical decision points:

```bash
TASK="{TASK_NAME}"; SID=$(cat /tmp/claude_task_registry/task_${TASK} 2>/dev/null || echo ""); USED=$([ -n "$SID" ] && cat /tmp/claude_tool_counts/${SID} 2>/dev/null || echo 0); echo "used=${USED} budget={TOOL_BUDGET} remaining=$((TOOL_BUDGET - USED - 1))"
```

Replace `{TASK_NAME}` and `{TOOL_BUDGET}` with actual values. The `-1` accounts for this Bash call itself. Run this command:

1. **Before starting Section J (self-check)** — if `remaining ≤ 2`, skip optional checks and go straight to reporting.
2. **Before any re-write** — if `remaining ≤ 1`, skip the re-write, note the issue in `difficulties`, and report.
...
**Before returning the report**, read the hook-measured count and write the audit file:

```bash
TASK="{TASK_NAME}"
SID=$(cat /tmp/claude_task_registry/task_${TASK} 2>/dev/null || echo "")
HOOK_COUNT=$([ -n "$SID" ] && cat /tmp/claude_tool_counts/${SID} 2>/dev/null || echo "unknown")
echo "$HOOK_COUNT" > /tmp/tool_audit_${TASK}
echo "$HOOK_COUNT"
```

Replace `{TASK_NAME}` with the actual value. The echoed number is your `tool_calls_used`. Do not estimate this field yourself.

PostToolUse fires *after* each tool, so this audit-read Bash call is counted by the hook only after you have already read the value — meaning `tool_calls_used` does NOT include this call. The orchestrator adds 1 to get the true total.
...
- `tool_calls_used`: integer — read from hook counter above; includes registration + actual work + budget-check calls; excludes this audit-read.
- `budget_check_count`: integer — how many times you ran the budget-check command (0, 1, or 2).
